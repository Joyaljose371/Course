...
// Draws a 1080x1080 branded "streak card" and resolves with a PNG Blob.
function generateShareImageBlob(profile, level) {
...
async function shareProgress(profile, level, setBusy, setError) {
  setError("");
  setBusy(true);
  try {
    ...
    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      ...
      // Fallback for browsers without native file sharing (mostly desktop): download the image.
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url; a.download = "psych-catalog-streak.png";
      document.body.appendChild(a); a.click(); a.remove();
      URL.revokeObjectURL(url);
      setError("Your browser can't open the share sheet directly — the image downloaded instead, so you can attach it manually.");
    }
  } catch (e) {
    if (e.name !== "AbortError") setError("Couldn't generate or share the image. Please try again.");
  } finally {
    setBusy(false);
  }
}

function ShareProgressButton({ profile, level }) {
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  return (
    <div>
      <GhostButton onClick={() => shareProgress(profile, level, setBusy, setError)} style={{ borderColor: T.brass, color: T.brassLight }}>
        <Share2 size={13} /> {busy ? "Preparing image…" : "Share streak"}
      </GhostButton>
      {error && <div style={{ fontFamily: FONT_BODY, fontSize: 11.5, color: T.textMuted, marginTop: 6, maxWidth: 240 }}>{error}</div>}
    </div>
  );
}

function Dashboard({ profile, dbData, setActive, setBrowseFilter, completeDailyPost, toggleSaveDaily }) {
  ...
  const todaysPost = pickTodaysPost(dbData.dailyPosts);
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 14, marginBottom: 20 }}>
        <div style={{ background: T.bgPanel, border: `1px solid ${T.bgPanelLight}`, borderRadius: 8, padding: 16, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 14 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            <div style={{ width: 44, height: 44, borderRadius: 10, background: `${T.brass}22`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <Flame size={22} color={T.brass} />
            </div>
            <div>
              <div style={{ fontFamily: FONT_DISPLAY, fontSize: 22, fontWeight: 700, color: T.textLight }}>{profile.streak.current} day{profile.streak.current === 1 ? "" : "s"}</div>
              <div style={{ fontFamily: FONT_BODY, fontSize: 12, color: T.textMuted }}>current streak · best {profile.streak.longest}</div>
            </div>
          </div>
          <ShareProgressButton profile={profile} level={level} />
        </div>
        <div style={{ background: T.bgPanel, border: `1px solid ${T.bgPanelLight}`, borderRadius: 8, padding: 16 }}>
      ...
      {profile.badges.length > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 24 }}>
          {profile.badges.map((badgeId) => (
            <span key={badgeId} style={{ display: "flex", alignItems: "center", gap: 5, background: `${T.brass}18`, border: `1px solid ${T.brass}55`, color: T.brassLight, borderRadius: 14, padding: "4px 10px", fontFamily: FONT_BODY, fontSize: 11.5, fontWeight: 600 }}>
              <Award size={12} /> {badgeLabel(badgeId)}
            </span>
          ))}
        </div>
      )}
      ...
      {todaysPost && (
        <div style={{ marginBottom: 32 }}>
          <SectionHeading eyebrow="Today's Psychology" title={todaysPost.title} />
          <DailyPsychologyCard post={todaysPost} profile={profile} completeDailyPost={completeDailyPost} toggleSaveDaily={toggleSaveDaily} />
        </div>
      )}
...
// Picks the most relevant Daily Psychology post: the one dated today if it
// exists, otherwise the most recent past post, so the card is never empty.
function pickTodaysPost(dailyPosts) {
  if (!dailyPosts || dailyPosts.length === 0) return null;
  const today = new Date().toISOString().slice(0, 10);
  const exact = dailyPosts.find((p) => p.date === today);
  if (exact) return exact;
  const past = dailyPosts.filter((p) => p.date <= today);
  return past.length ? past[past.length - 1] : dailyPosts[0];
}

function badgeLabel(badgeId) {
  const map = { streak_3: "3-Day Streak", streak_7: "7-Day Streak", streak_14: "14-Day Streak", streak_30: "30-Day Streak" };
  return map[badgeId] || badgeId;
}

function DailyPsychologyCard({ post, profile, completeDailyPost, toggleSaveDaily }) {
  const [expanded, setExpanded] = useState(false);
  const [selectedOpt, setSelectedOpt] = useState(null);
  const isRead = profile.dailyRead.includes(post.id);
  const isSaved = profile.dailySaved.includes(post.id);

  const selectOption = (i) => {
    if (selectedOpt !== null) return;
    setSelectedOpt(i);
    completeDailyPost(post.id, post.xp || 40);
  };

  return (
    <div style={{ background: T.paper, borderRadius: 10, padding: "22px 24px", color: T.ink, boxShadow: "0 8px 20px rgba(0,0,0,0.25)", maxWidth: 620 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
        <CatalogStamp>Read in 3 minutes</CatalogStamp>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          {isRead && <span style={{ display: "flex", alignItems: "center", gap: 4, fontFamily: FONT_BODY, fontSize: 11.5, color: T.sage }}><CheckCircle2 size={13} /> done</span>}
          <button onClick={() => toggleSaveDaily(post.id)} className="psy-focus" style={{ background: "none", border: "none", cursor: "pointer", padding: 2, color: isSaved ? T.brass : T.inkSoft }} aria-label="Save">
            {isSaved ? <BookmarkCheck size={16} /> : <Bookmark size={16} />}
          </button>
        </div>
      </div>
      <p style={{ fontFamily: FONT_BODY, fontSize: 14.5, lineHeight: 1.7, margin: "14px 0 0" }}>{post.body}</p>
      {!expanded ? (
        <button onClick={() => setExpanded(true)} className="psy-focus" style={{ marginTop: 12, background: "none", border: "none", color: T.brass, fontFamily: FONT_BODY, fontWeight: 700, fontSize: 13, cursor: "pointer", padding: 0 }}>
          Read example + research →
        </button>
      ) : (
        <>
          <div style={{ marginTop: 16, paddingTop: 14, borderTop: `1px solid ${T.ink}22` }}>
            <div style={{ fontFamily: FONT_MONO, fontSize: 10.5, letterSpacing: "0.08em", textTransform: "uppercase", color: T.inkSoft, marginBottom: 6 }}>Real-life example</div>
            <p style={{ fontFamily: FONT_BODY, fontSize: 14, lineHeight: 1.65, margin: 0 }}>{post.examples}</p>
          </div>
          <div style={{ marginTop: 14 }}>
            <div style={{ fontFamily: FONT_MONO, fontSize: 10.5, letterSpacing: "0.08em", textTransform: "uppercase", color: T.inkSoft, marginBottom: 6 }}>Research</div>
            <p style={{ fontFamily: FONT_BODY, fontSize: 14, lineHeight: 1.65, margin: 0 }}>{post.research}</p>
          </div>
          {post.quiz && (
            <div style={{ marginTop: 18, paddingTop: 16, borderTop: `1px solid ${T.ink}22` }}>
              <div style={{ fontFamily: FONT_MONO, fontSize: 10.5, letterSpacing: "0.08em", textTransform: "uppercase", color: T.brass, marginBottom: 8 }}>Mini quiz · +{post.xp || 40} XP</div>
              <div style={{ fontFamily: FONT_BODY, fontWeight: 600, fontSize: 14.5, marginBottom: 10 }}>{post.quiz.question}</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {post.quiz.options.map((opt, i) => {
                  const isSelected = selectedOpt === i;
                  const isCorrect = i === post.quiz.correct;
                  let bg = "transparent", border = `${T.ink}33`;
                  if (selectedOpt !== null) {
                    if (isCorrect) { bg = `${T.sage}33`; border = T.sage; }
                    else if (isSelected) { bg = `${T.rust}22`; border = T.rust; }
                  }
                  return (
                    <button key={i} onClick={() => selectOption(i)} className="psy-focus" style={{ textAlign: "left", padding: "9px 12px", borderRadius: 7, border: `1px solid ${border}`, background: bg, color: T.ink, fontFamily: FONT_BODY, fontSize: 13.5, cursor: selectedOpt === null ? "pointer" : "default" }}>
                      {opt}
                    </button>
                  );
                })}
              </div>
              {selectedOpt !== null && <div style={{ marginTop: 10, fontFamily: FONT_BODY, fontSize: 13, color: T.inkSoft, lineHeight: 1.6 }}>{post.quiz.explanation}</div>}
              {selectedOpt !== null && (
                <PrimaryButton onClick={() => setExpanded(false)} style={{ marginTop: 14 }}>
...
const RESEARCH_ALGORITHM = [
  { step: 1, title: "Ask a clear question", body: "Start with something you're genuinely curious about, narrowed down enough to actually study — not \"does stress matter?\" but \"does sleep loss affect memory in college students?\"" },
  { step: 2, title: "Read what's already known", body: "Search existing studies (Google Scholar, PubMed, APA PsycNet — see the Explore tab) so you're not repeating work, and so you can build on what others already found." },
  { step: 3, title: "Turn it into a hypothesis", body: "Write one specific, testable prediction — e.g., \"People who sleep 4 hours will recall fewer words than people who sleep 8 hours.\" If you can't imagine a result that would prove it wrong, it's not testable yet." },
  { step: 4, title: "Pick how you'll test it", body: "Experiment (you control something and measure the effect), survey (you ask questions), or observation (you watch without interfering). Your question decides which fits — you can't randomly assign people to \"grow up poor\" or \"grow up wealthy,\" for example." },
  { step: 5, title: "Decide who you'll study", body: "How many people, and how will you pick them fairly? A random or well-matched sample lets your results apply beyond just the people in the room." },
  { step: 6, title: "Collect your data", body: "Run the study the same way for everyone. Consistency here is what makes the comparison between groups fair and meaningful." },
  { step: 7, title: "Run the numbers", body: "Use statistics to check whether the pattern you found is a real effect or could easily have happened by chance. This is the difference between a story and a finding." },
  { step: 8, title: "Write it up and share it", body: "Explain what you did, what you found, and what it means — clearly enough that someone else could repeat it. That's what actually makes it science instead of just an opinion." },
];

function ResearchGettingStarted() {
  return (
    <div style={{ background: T.bgPanel, border: `1px solid ${T.bgPanelLight}`, borderRadius: 10, padding: "22px 22px 6px", marginBottom: 26 }}>
      <div style={{ fontFamily: FONT_MONO, fontSize: 11, letterSpacing: "0.12em", textTransform: "uppercase", color: T.brass, marginBottom: 4 }}>New to research? Start here</div>
      <h3 style={{ fontFamily: FONT_DISPLAY, fontWeight: 700, fontSize: 20, color: T.textLight, margin: "0 0 6px" }}>How psychology research actually works</h3>
      <p style={{ fontFamily: FONT_BODY, fontSize: 13, color: T.textMuted, margin: "0 0 20px", lineHeight: 1.6, maxWidth: 640 }}>
        Underneath all the terminology below, every study follows roughly the same eight-step process. Read this once, plain and simple, before diving into the detailed ledger.
      </p>
      <div style={{ position: "relative", paddingLeft: 4 }}>
        {RESEARCH_ALGORITHM.map((s, i) => (
          <div key={s.step} style={{ display: "flex", gap: 16, position: "relative", paddingBottom: i === RESEARCH_ALGORITHM.length - 1 ? 20 : 26 }}>
            ...
            <div style={{ paddingTop: 3 }}>
              <div style={{ fontFamily: FONT_DISPLAY, fontWeight: 700, fontSize: 15.5, color: T.textLight, marginBottom: 4 }}>{s.title}</div>
              <div style={{ fontFamily: FONT_BODY, fontSize: 13.5, color: T.textMuted, lineHeight: 1.65, maxWidth: 580 }}>{s.body}</div>
            </div>
          </div>
        ))}
      </div>
...
function ResearchMethodology({ dbData }) {
  ...
  return (
    <div>
      ...
      <div style={{ fontFamily: FONT_MONO, fontSize: 11, letterSpacing: "0.12em", textTransform: "uppercase", color: T.textMuted, margin: "4px 0 12px" }}>The detailed ledger — exam-level depth</div>
      ...
      <div style={{ background: T.paper, borderRadius: 10, padding: 24, maxWidth: 560, color: T.ink }}>
        ...
        {q.scenario && (
          <div style={{ background: `${T.ink}0D`, border: `1px solid ${T.ink}22`, borderRadius: 8, padding: "12px 14px", margin: "14px 0" }}>
            <div style={{ fontFamily: FONT_MONO, fontSize: 10.5, letterSpacing: "0.08em", textTransform: "uppercase", color: T.inkSoft, marginBottom: 6 }}>Scenario</div>
            <div style={{ fontFamily: FONT_BODY, fontSize: 13.5, lineHeight: 1.6, fontStyle: "italic" }}>{q.scenario}</div>
          </div>
        )}
  ...
  return (
    <div>
      ...
      {list.length === 0 ? <EmptyState text="No case studies in this category yet." /> : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 14 }}>
          {list.map((cs) => {
            const cat = catOf(cs.categoryId);
            const isDone = profile.completedCaseStudies.includes(cs.id);
            return (
              <IndexCard key={cs.id} tint={cat.tint} onClick={() => setSelected(cs)}>
                <CatalogStamp tint={cat.tint}>{cat.name}</CatalogStamp>
                <div style={{ fontFamily: FONT_DISPLAY, fontWeight: 600, fontSize: 16.5, marginTop: 10, lineHeight: 1.3 }}>{cs.title}</div>
                <div style={{ fontFamily: FONT_BODY, fontSize: 12.5, color: T.inkSoft, marginTop: 8, lineHeight: 1.5 }}>{cs.scenario.slice(0, 90)}…</div>
                {isDone && <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 10, fontFamily: FONT_BODY, fontSize: 11.5, color: T.sage }}><CheckCircle2 size={13} /> completed</div>}
              </IndexCard>
            );
          })}
        </div>
...
const CASE_STAGES = [
  { key: "diagnosis", label: "Diagnosis", optKey: "diagnosisOptions", correctKey: "diagnosisCorrect", explKey: "diagnosisExplanation" },
  { key: "cause", label: "Possible Cause", optKey: "causeOptions", correctKey: "causeCorrect", explKey: "causeExplanation" },
  { key: "therapy", label: "Therapy", optKey: "therapyOptions", correctKey: "therapyCorrect", explKey: "therapyExplanation" },
];

function CaseStudyDetail({ caseStudy, cat, onBack, completeCaseStudy }) {
  const [stageIndex, setStageIndex] = useState(0);
  const [answers, setAnswers] = useState({});
  const [finished, setFinished] = useState(false);

  const stage = CASE_STAGES[stageIndex];
  const selectedOpt = answers[stage.key];
  const options = caseStudy[stage.optKey] || [];
  const correct = caseStudy[stage.correctKey];
  const explanation = caseStudy[stage.explKey];

  const selectOption = (i) => {
    if (selectedOpt !== undefined) return;
    setAnswers((a) => ({ ...a, [stage.key]: i }));
  };

  const next = () => {
    if (stageIndex + 1 < CASE_STAGES.length) {
      setStageIndex(stageIndex + 1);
    } else {
      completeCaseStudy(caseStudy.id, caseStudy.xp || 25);
      setFinished(true);
    }
  };

  if (finished) {
    const correctCount = CASE_STAGES.filter((s) => answers[s.key] === caseStudy[s.correctKey]).length;
    return (
      <div>
        ...
        <div style={{ background: T.paper, borderRadius: 10, padding: "30px 26px", color: T.ink, maxWidth: 480, textAlign: "center", boxShadow: "0 8px 20px rgba(0,0,0,0.25)" }}>
          <Award size={30} color={T.brass} style={{ margin: "0 auto 12px" }} />
          <div style={{ fontFamily: FONT_MONO, fontSize: 11, letterSpacing: "0.1em", textTransform: "uppercase", color: T.inkSoft, marginBottom: 6 }}>Case complete</div>
          ...
          <div style={{ marginTop: 22 }}>
            <PrimaryButton onClick={onBack} style={{ margin: "0 auto" }}>Back to case studies <ChevronRight size={14} /></PrimaryButton>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div>
      ...
      <div style={{ background: T.paper, borderRadius: 10, padding: "24px 26px", color: T.ink, maxWidth: 600, boxShadow: "0 8px 20px rgba(0,0,0,0.25)" }}>
        ...
        <div style={{ fontFamily: FONT_DISPLAY, fontWeight: 600, fontSize: 16.5, marginBottom: 12 }}>What's the most likely {stage.label.toLowerCase()}?</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
          {options.map((opt, i) => {
            const isSelected = selectedOpt === i, isCorrect = i === correct;
            let bg = "transparent", border = `${T.ink}33`;
            ...
            return (
              <button key={i} onClick={() => selectOption(i)} className="psy-focus" style={{ textAlign: "left", padding: "10px 14px", borderRadius: 8, border: `1px solid ${border}`, background: bg, color: T.ink, fontFamily: FONT_BODY, fontSize: 14, cursor: selectedOpt === undefined ? "pointer" : "default", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                {opt}
                {selectedOpt !== undefined && isCorrect && <CheckCircle2 size={16} color={T.sage} />}
                {selectedOpt !== undefined && isSelected && !isCorrect && <XCircle size={16} color={T.rust} />}
              </button>
            );
          })}
        </div>
        {selectedOpt !== undefined && <div style={{ marginTop: 16, fontFamily: FONT_BODY, fontSize: 13, lineHeight: 1.6, color: T.inkSoft, borderTop: `1px solid ${T.ink}22`, paddingTop: 14 }}>{explanation}</div>}
        {selectedOpt !== undefined && (
          <PrimaryButton onClick={next} style={{ marginTop: 18 }}>
            {stageIndex + 1 < CASE_STAGES.length ? "Next: " + CASE_STAGES[stageIndex + 1].label : "Finish case"} <ChevronRight size={14} />
          </PrimaryButton>
        )}
      </div>
    </div>
...
/* ============================== EXPLORE (JOURNALS & RESEARCH DIRECTORY) ============================== */
const JOURNAL_TYPES = ["Journal", "Database", "Open Access"];

function Explore({ dbData }) {
  const [filterCat, setFilterCat] = useState("all");
  const [filterType, setFilterType] = useState("all");
  const [query, setQuery] = useState("");

  const catOf = (id) => {
    if (!id) return { name: "General", tint: T.textMuted };
    return dbData.categories.find((c) => c.id === id) || { name: id, tint: T.brass };
  };

  const list = useMemo(() => {
    return dbData.journals.filter((j) => {
      const matchesCat = filterCat === "all" || (filterCat === "general" ? !j.categoryId : j.categoryId === filterCat);
      const matchesType = filterType === "all" || j.type === filterType;
      const q = query.trim().toLowerCase();
      const matchesQuery = q === "" || j.title.toLowerCase().includes(q) || (j.description || "").toLowerCase().includes(q) || (j.source || "").toLowerCase().includes(q);
      return matchesCat && matchesType && matchesQuery;
    }).sort((a, b) => (b.recent ? 1 : 0) - (a.recent ? 1 : 0));
  }, [dbData.journals, filterCat, filterType, query]);

  return (
    <div>
      <SectionHeading eyebrow="Research Directory" title="Explore" right={
        <div style={{ display: "flex", alignItems: "center", gap: 8, background: T.bgPanel, border: `1px solid ${T.bgPanelLight}`, borderRadius: 8, padding: "7px 10px" }}>
          <Search size={14} color={T.textMuted} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search journals, databases..." className="psy-focus" style={{ background: "transparent", border: "none", outline: "none", color: T.textLight, fontFamily: FONT_BODY, fontSize: 13, width: 200 }} />
        </div>
      } />
      <p style={{ fontFamily: FONT_BODY, fontSize: 12.5, color: T.textMuted, marginBottom: 18, maxWidth: 640, lineHeight: 1.6 }}>
...
const ADMIN_TABS = [
  ...
  { id: "dailyPosts", label: "Daily Psychology" }, { id: "caseStudies", label: "Case Studies" },
  { id: "journals", label: "Explore / Journals" },
  { id: "importExport", label: "Import / Export" },
];

function AdminSectionGate({ canAccess, sectionLabel, children }) {
  if (canAccess) return children;
  return (
    <div style={{ background: T.bgPanel, border: `1px solid ${T.bgPanelLight}`, borderRadius: 10, padding: 16, color: T.textMuted, fontFamily: FONT_BODY, lineHeight: 1.6 }}>
      You do not currently have permission to manage {sectionLabel}. Ask a super-admin to grant this access.
    </div>
  );
}
...
function AdminPanel({ dbData, addItem, updateItem, deleteItem, deleteCategory, seedStarterContent, exportContent, importContent, adminData, admins, saveAdmin, removeAdmin, currentUid }) {
  ...
  const canManageSection = (key) => isSuperAdmin || Boolean(adminData?.permissions?.[key]);
  ...
  return (
    <div>
      ...
      {tab === "quiz" && <AdminSectionGate canAccess={canManageSection("quiz")} sectionLabel="quiz questions"><AdminQuiz dbData={dbData} addItem={addItem} updateItem={updateItem} deleteItem={deleteItem} /></AdminSectionGate>}
      {tab === "flashcards" && <AdminSectionGate canAccess={canManageSection("flashcards")} sectionLabel="flashcards"><AdminFlashcards dbData={dbData} addItem={addItem} updateItem={updateItem} deleteItem={deleteItem} /></AdminSectionGate>}
      {tab === "research" && <AdminSectionGate canAccess={canManageSection("research")} sectionLabel="research sections"><AdminResearch dbData={dbData} addItem={addItem} updateItem={updateItem} deleteItem={deleteItem} /></AdminSectionGate>}
      {tab === "dailyPosts" && <AdminSectionGate canAccess={canManageSection("dailyPosts")} sectionLabel="daily posts"><AdminDailyPosts dbData={dbData} addItem={addItem} updateItem={updateItem} deleteItem={deleteItem} /></AdminSectionGate>}
...
function AdminDirectoryPanel({ admins, saveAdmin, removeAdmin, currentUid, currentAdminData }) {
  ...
  const startEdit = (admin) => {
    ...
    setPermissions({ ...DEFAULT_ADMIN_PERMISSIONS, ...(admin.permissions || {}) });
  };
  ...
  const submit = async (e) => {
    ...
    try {
      await saveAdmin(uid.trim(), { role, permissions });
      resetForm();
      setMessage(editingUid ? "Admin updated." : "Admin saved.");
    } catch (err) {
      setMessage(err.message || "Could not save this admin.");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "minmax(280px, 360px) 1fr", gap: 20, alignItems: "start" }}>
      <div style={{ background: T.bgPanel, border: `1px solid ${T.bgPanelLight}`, borderRadius: 10, padding: 18 }}>
        <div style={{ fontFamily: FONT_DISPLAY, fontWeight: 600, fontSize: 16, color: T.textLight, marginBottom: 14 }}>{editingUid ? "Edit admin access" : "Create a new admin"}</div>
        <form onSubmit={submit}>
          <Field label="User UID"><TextInput value={uid} onChange={(e) => setUid(e.target.value)} placeholder="Firebase Auth UID" /></Field>
          <Field label="Role">
          ...
          <div style={{ fontFamily: FONT_BODY, fontSize: 12.5, color: T.textMuted, marginBottom: 10 }}>
            Choose which content sections this admin can edit. Super-admins can manage everything and all other admins.
          </div>
          <div style={{ display: "grid", gap: 8, marginBottom: 12 }}>
            {[
              ["categories", "Categories"], ["topics", "Topics"], ["persons", "Theories & people"], ["quiz", "Quiz"], ["flashcards", "Flashcards"], ["research", "Research"], ["dailyPosts", "Daily posts"], ["caseStudies", "Case studies"],
            ].map(([key, label]) => (
              <label key={key} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: `${T.bgDeep}88`, borderRadius: 8, padding: "8px 10px", color: T.textLight, fontFamily: FONT_BODY, fontSize: 13 }}>
                <span>{label}</span>
                <input type="checkbox" checked={permissions[key]} onChange={() => togglePermission(key)} />
              </label>
            ))}
            <label style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: `${T.bgDeep}88`, borderRadius: 8, padding: "8px 10px", color: T.textLight, fontFamily: FONT_BODY, fontSize: 13 }}>
              <span>Import / Export</span>
              <input type="checkbox" checked={permissions.canImportExport} onChange={() => togglePermission("canImportExport")} />
            </label>
            <label style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: `${T.bgDeep}88`, borderRadius: 8, padding: "8px 10px", color: T.textLight, fontFamily: FONT_BODY, fontSize: 13 }}>
              <span>Manage admins</span>
              <input type="checkbox" checked={permissions.canManageAdmins} onChange={() => togglePermission("canManageAdmins")} />
            </label>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <PrimaryButton type="submit" disabled={busy}>{busy ? "Saving…" : editingUid ? "Save changes" : "Create admin"}</PrimaryButton>
            {(editingUid || uid) && <GhostButton onClick={resetForm}>Clear</GhostButton>}
          </div>
          {message && <div style={{ marginTop: 10, fontFamily: FONT_BODY, fontSize: 12.5, color: T.brassLight }}>{message}</div>}
        </form>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {admins.length === 0 ? <EmptyState text="No admins yet." /> : admins.map((admin) => (
          <div key={admin.uid} style={{ background: T.bgPanel, border: `1px solid ${T.bgPanelLight}`, borderRadius: 8, padding: "12px 14px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
              <div>
                <div style={{ fontFamily: FONT_DISPLAY, fontWeight: 600, fontSize: 15, color: T.textLight }}>{admin.role === "super_admin" ? "Super admin" : "Admin"}</div>
                <div style={{ fontFamily: FONT_MONO, fontSize: 11, color: T.textMuted, marginTop: 4, wordBreak: "break-all" }}>{admin.uid}</div>
                <div style={{ fontFamily: FONT_BODY, fontSize: 12.5, color: T.textMuted, marginTop: 8 }}>
                  {admin.role === "super_admin" ? "Full access to all sections and admin management" : `${Object.entries(admin.permissions || {}).filter(([,value]) => value).map(([key]) => key).join(", ") || "No extra permissions"}`}
                </div>
              </div>
              <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                <GhostButton onClick={() => startEdit(admin)}><Pencil size={13} /> Edit</GhostButton>
                <GhostButton danger disabled={admin.uid === currentUid} onClick={() => { if (confirm("Remove this admin?")) removeAdmin(admin.uid); }}><Trash2 size={13} /></GhostButton>
              </div>
            </div>
          </div>
        ))}
...
function AdminDailyPosts({ dbData, addItem, updateItem, deleteItem }) {
  const blank = { date: "", title: "", body: "", examples: "", research: "", quizQuestion: "", quizOpt0: "", quizOpt1: "", quizOpt2: "", quizOpt3: "", quizCorrect: "0", quizExplanation: "", xp: "40" };
  const { form, setForm, editingId, setEditingId, reset } = useEditableForm(blank);

  const startEdit = (p) => {
    setForm({
      date: p.date || "", title: p.title || "", body: p.body || "", examples: p.examples || "", research: p.research || "",
      quizQuestion: p.quiz?.question || "", quizOpt0: p.quiz?.options?.[0] || "", quizOpt1: p.quiz?.options?.[1] || "",
      quizOpt2: p.quiz?.options?.[2] || "", quizOpt3: p.quiz?.options?.[3] || "", quizCorrect: String(p.quiz?.correct ?? 0),
      quizExplanation: p.quiz?.explanation || "", xp: String(p.xp ?? 40),
    });
    setEditingId(p.id);
  };
  ...
  const submit = (e) => {
    ...
    const hasQuiz = form.quizQuestion.trim() !== "";
    const payload = {
      date: form.date.trim(),
      ...
      xp: parseInt(form.xp, 10) || 40,
      quiz: hasQuiz ? {
        question: form.quizQuestion.trim(),
        options: [form.quizOpt0, form.quizOpt1, form.quizOpt2, form.quizOpt3].map((s) => s.trim()),
        correct: parseInt(form.quizCorrect, 10),
        explanation: form.quizExplanation.trim(),
      } : null,
    };
    if (editingId) updateItem("dailyPosts", editingId, payload); else addItem("dailyPosts", payload);
    reset();
  };

  return (
    <AdminShell formTitle={editingId ? "Edit daily post" : "Add a Daily Psychology post"} form={
      <form onSubmit={submit}>
        <Field label="Date (YYYY-MM-DD) — shown on this date"><TextInput type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} /></Field>
        <Field label="Title"><TextInput value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="e.g. Confirmation Bias" /></Field>
        <Field label="Body (main 3-minute explanation)"><TextArea value={form.body} onChange={(e) => setForm({ ...form, body: e.target.value })} /></Field>
        <Field label="Real-life example"><TextArea value={form.examples} onChange={(e) => setForm({ ...form, examples: e.target.value })} /></Field>
        <Field label="Research note"><TextArea value={form.research} onChange={(e) => setForm({ ...form, research: e.target.value })} /></Field>
        <Field label="Mini-quiz question (optional)"><TextArea value={form.quizQuestion} onChange={(e) => setForm({ ...form, quizQuestion: e.target.value })} placeholder="Leave blank for no mini-quiz" /></Field>
        <Field label="Option A"><TextInput value={form.quizOpt0} onChange={(e) => setForm({ ...form, quizOpt0: e.target.value })} /></Field>
        <Field label="Option B"><TextInput value={form.quizOpt1} onChange={(e) => setForm({ ...form, quizOpt1: e.target.value })} /></Field>
        <Field label="Option C"><TextInput value={form.quizOpt2} onChange={(e) => setForm({ ...form, quizOpt2: e.target.value })} /></Field>
        <Field label="Option D"><TextInput value={form.quizOpt3} onChange={(e) => setForm({ ...form, quizOpt3: e.target.value })} /></Field>
        <Field label="Correct answer"><Select value={form.quizCorrect} onChange={(e) => setForm({ ...form, quizCorrect: e.target.value })}><option value="0">A</option><option value="1">B</option><option value="2">C</option><option value="3">D</option></Select></Field>
        <Field label="Explanation"><TextArea value={form.quizExplanation} onChange={(e) => setForm({ ...form, quizExplanation: e.target.value })} /></Field>
        <Field label="XP awarded for completing the mini-quiz"><TextInput type="number" value={form.xp} onChange={(e) => setForm({ ...form, xp: e.target.value })} /></Field>
        <div style={{ display: "flex", gap: 8 }}><PrimaryButton type="submit">{editingId ? <><Save size={14} /> Save</> : <><Plus size={14} /> Add post</>}</PrimaryButton>{editingId && <GhostButton onClick={reset}>Cancel</GhostButton>}</div>
      </form>
    } list={dbData.dailyPosts.map((p) => (
      <div key={p.id} style={{ background: T.bgPanel, border: `1px solid ${T.bgPanelLight}`, borderRadius: 8, padding: "12px 14px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
          <div>
            <span style={{ fontFamily: FONT_MONO, fontSize: 11, color: T.brass }}>{p.date}</span>
            <div style={{ fontFamily: FONT_DISPLAY, fontWeight: 600, fontSize: 15, color: T.textLight, marginTop: 4 }}>{p.title}</div>
          </div>
          <div style={{ display: "flex", gap: 6 }}><GhostButton onClick={() => startEdit(p)}><Pencil size={13} /> Edit</GhostButton><GhostButton danger onClick={() => { if (confirm(`Delete "${p.title}"?`)) deleteItem("dailyPosts", p.id); }}><Trash2 size={13} /></GhostButton></div>
        </div>
      </div>
    ))} />
  );
...
function AdminCaseStudies({ dbData, addItem, updateItem, deleteItem }) {
  const blank = {
    categoryId: dbData.categories[0]?.id || "", title: "", scenario: "", xp: "25",
    diagOpt0: "", diagOpt1: "", diagOpt2: "", diagOpt3: "", diagCorrect: "0", diagExpl: "",
    causeOpt0: "", causeOpt1: "", causeOpt2: "", causeOpt3: "", causeCorrect: "0", causeExpl: "",
    therapyOpt0: "", therapyOpt1: "", therapyOpt2: "", therapyOpt3: "", therapyCorrect: "0", therapyExpl: "",
  };
  const { form, setForm, editingId, setEditingId, reset } = useEditableForm(blank);

  const startEdit = (cs) => {
    setForm({
      categoryId: cs.categoryId, title: cs.title, scenario: cs.scenario, xp: String(cs.xp ?? 25),
      diagOpt0: cs.diagnosisOptions?.[0] || "", diagOpt1: cs.diagnosisOptions?.[1] || "", diagOpt2: cs.diagnosisOptions?.[2] || "", diagOpt3: cs.diagnosisOptions?.[3] || "",
      diagCorrect: String(cs.diagnosisCorrect ?? 0), diagExpl: cs.diagnosisExplanation || "",
      causeOpt0: cs.causeOptions?.[0] || "", causeOpt1: cs.causeOptions?.[1] || "", causeOpt2: cs.causeOptions?.[2] || "", causeOpt3: cs.causeOptions?.[3] || "",
      causeCorrect: String(cs.causeCorrect ?? 0), causeExpl: cs.causeExplanation || "",
      therapyOpt0: cs.therapyOptions?.[0] || "", therapyOpt1: cs.therapyOptions?.[1] || "", therapyOpt2: cs.therapyOptions?.[2] || "", therapyOpt3: cs.therapyOptions?.[3] || "",
      therapyCorrect: String(cs.therapyCorrect ?? 0), therapyExpl: cs.therapyExplanation || "",
    });
    setEditingId(cs.id);
  };
  ...
  const submit = (e) => {
    e.preventDefault();
    if (!form.title.trim() || !form.scenario.trim()) return;
    const payload = {
      categoryId: form.categoryId, title: form.title.trim(), scenario: form.scenario.trim(), xp: parseInt(form.xp, 10) || 25,
      diagnosisOptions: [form.diagOpt0, form.diagOpt1, form.diagOpt2, form.diagOpt3].map((s) => s.trim()),
      diagnosisCorrect: parseInt(form.diagCorrect, 10), diagnosisExplanation: form.diagExpl.trim(),
      causeOptions: [form.causeOpt0, form.causeOpt1, form.causeOpt2, form.causeOpt3].map((s) => s.trim()),
      causeCorrect: parseInt(form.causeCorrect, 10), causeExplanation: form.causeExpl.trim(),
      therapyOptions: [form.therapyOpt0, form.therapyOpt1, form.therapyOpt2, form.therapyOpt3].map((s) => s.trim()),
      therapyCorrect: parseInt(form.therapyCorrect, 10), therapyExplanation: form.therapyExpl.trim(),
    };
    if (editingId) updateItem("caseStudies", editingId, payload); else addItem("caseStudies", payload);
    reset();
  };

  const stageFields = (prefix, label) => (
    <div style={{ marginTop: 14, paddingTop: 14, borderTop: `1px solid ${T.bgPanelLight}` }}>
      <div style={{ fontFamily: FONT_DISPLAY, fontWeight: 700, fontSize: 13.5, color: T.brassLight, marginBottom: 8 }}>{label}</div>
      <Field label="Option A"><TextInput value={form[`${prefix}Opt0`]} onChange={(e) => setForm({ ...form, [`${prefix}Opt0`]: e.target.value })} /></Field>
      <Field label="Option B"><TextInput value={form[`${prefix}Opt1`]} onChange={(e) => setForm({ ...form, [`${prefix}Opt1`]: e.target.value })} /></Field>
      <Field label="Option C"><TextInput value={form[`${prefix}Opt2`]} onChange={(e) => setForm({ ...form, [`${prefix}Opt2`]: e.target.value })} /></Field>
      <Field label="Option D"><TextInput value={form[`${prefix}Opt3`]} onChange={(e) => setForm({ ...form, [`${prefix}Opt3`]: e.target.value })} /></Field>
      <Field label="Correct answer"><Select value={form[`${prefix}Correct`]} onChange={(e) => setForm({ ...form, [`${prefix}Correct`]: e.target.value })}><option value="0">A</option><option value="1">B</option><option value="2">C</option><option value="3">D</option></Select></Field>
      <Field label="Explanation"><TextArea value={form[`${prefix}Expl`]} onChange={(e) => setForm({ ...form, [`${prefix}Expl`]: e.target.value })} /></Field>
    </div>
  );

  return (
    <AdminShell formTitle={editingId ? "Edit case study" : "Add a case study"} form={
      <form onSubmit={submit}>
        ...
        <Field label="Title"><TextInput value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="e.g. Panic Before Presentations" /></Field>
        <Field label="Scenario"><TextArea value={form.scenario} onChange={(e) => setForm({ ...form, scenario: e.target.value })} placeholder="Describe the case" /></Field>
        <Field label="XP awarded on completion"><TextInput type="number" value={form.xp} onChange={(e) => setForm({ ...form, xp: e.target.value })} /></Field>
        {stageFields("diag", "Stage 1 — Diagnosis")}
        {stageFields("cause", "Stage 2 — Possible Cause")}
        {stageFields("therapy", "Stage 3 — Therapy")}
        <div style={{ display: "flex", gap: 8, marginTop: 14 }}><PrimaryButton type="submit">{editingId ? <><Save size={14} /> Save</> : <><Plus size={14} /> Add case study</>}</PrimaryButton>{editingId && <GhostButton onClick={reset}>Cancel</GhostButton>}</div>
      </form>
    } list={dbData.caseStudies.map((cs) => {
      const cat = dbData.categories.find((c) => c.id === cs.categoryId);
      return (
        <div key={cs.id} style={{ background: T.bgPanel, border: `1px solid ${T.bgPanelLight}`, borderRadius: 8, padding: "12px 14px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
            <div>
              <CatalogStamp tint={cat?.tint}>{cat?.name || "—"}</CatalogStamp>
              <div style={{ fontFamily: FONT_DISPLAY, fontWeight: 600, fontSize: 15, color: T.textLight, marginTop: 6 }}>{cs.title}</div>
            </div>
            <div style={{ display: "flex", gap: 6 }}><GhostButton onClick={() => startEdit(cs)}><Pencil size={13} /> Edit</GhostButton><GhostButton danger onClick={() => { if (confirm(`Delete "${cs.title}"?`)) deleteItem("caseStudies", cs.id); }}><Trash2 size={13} /></GhostButton></div>
          </div>
        </div>
      );
    })} />
...
function AdminJournals({ dbData, addItem, updateItem, deleteItem }) {
  const blank = { categoryId: "", title: "", source: "", type: "Journal", url: "", description: "", recent: false };
  const { form, setForm, editingId, setEditingId, reset } = useEditableForm(blank);

  const startEdit = (j) => {
    setForm({ categoryId: j.categoryId || "", title: j.title, source: j.source || "", type: j.type || "Journal", url: j.url, description: j.description || "", recent: Boolean(j.recent) });
    setEditingId(j.id);
  };
  ...
  const submit = (e) => {
    ...
    const payload = {
      ...
      recent: Boolean(form.recent),
    };
    if (editingId) updateItem("journals", editingId, payload); else addItem("journals", payload);
    reset();
  };

  return (
    <AdminShell formTitle={editingId ? "Edit resource" : "Add a journal / database link"} form={
      <form onSubmit={submit}>
        <Field label="Category (optional — leave blank for General)">
        ...
        <Field label="Title"><TextInput value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="e.g. Frontiers in Psychology" /></Field>
        <Field label="Source / Publisher"><TextInput value={form.source} onChange={(e) => setForm({ ...form, source: e.target.value })} placeholder="e.g. Frontiers" /></Field>
        <Field label="Type">
        ...
        <Field label="Description"><TextArea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="What is this resource useful for?" /></Field>
        <Field label="">
          <label style={{ display: "flex", alignItems: "center", gap: 8, fontFamily: FONT_BODY, fontSize: 13, color: T.textLight, cursor: "pointer" }}>
            <input type="checkbox" checked={form.recent} onChange={(e) => setForm({ ...form, recent: e.target.checked })} /> Mark as "New"
          </label>
        </Field>
        <div style={{ display: "flex", gap: 8 }}><PrimaryButton type="submit">{editingId ? <><Save size={14} /> Save</> : <><Plus size={14} /> Add resource</>}</PrimaryButton>{editingId && <GhostButton onClick={reset}>Cancel</GhostButton>}</div>
      </form>
    } list={dbData.journals.map((j) => {
      const cat = j.categoryId ? dbData.categories.find((c) => c.id === j.categoryId) : null;
      return (
        <div key={j.id} style={{ background: T.bgPanel, border: `1px solid ${T.bgPanelLight}`, borderRadius: 8, padding: "12px 14px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
            <div>
...
function AdminQuiz({ dbData, addItem, updateItem, deleteItem }) {
  ...
  const submit = (e) => {
    ...
    const rawOpts = [form.opt0, form.opt1, form.opt2, form.opt3];
  ...
  return (
    <AdminShell formTitle={editingId ? "Edit question" : "Add a quiz question"} form={
      <form onSubmit={submit}>
        ...
        <Field label="Option C (leave blank for a True/False question)"><TextInput value={form.opt2} onChange={(e) => setForm({ ...form, opt2: e.target.value })} /></Field>
        <Field label="Option D (leave blank for a True/False question)"><TextInput value={form.opt3} onChange={(e) => setForm({ ...form, opt3: e.target.value })} /></Field>
        ...
        <div key={q.id} style={{ background: T.bgPanel, border: `1px solid ${T.bgPanelLight}`, borderRadius: 8, padding: "12px 14px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
            <div>
              ...
              <div style={{ fontFamily: FONT_BODY, fontSize: 14, color: T.textLight, marginTop: 6, lineHeight: 1.4 }}>{q.question}</div>
            </div>
